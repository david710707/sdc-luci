...
module("luci.controller.sdc.system", package.seeall)
...
function index()
	...
	entry({"sdc", "system", "clock"}, cbi("sdc_system/clock"), _("Clock"), 2)
	entry({"sdc", "system", "ntp"}, cbi("sdc_system/ntp"), _("NTP"), 3)
	entry({"sdc", "system", "admin"}, cbi("sdc_system/admin"), _("Administration"), 4)
	...
	entry({"sdc", "system", "reboot"}, call("action_reboot"), _("Reboot"), 90)
end
...
function action_packages()
	...
	luci.template.render("sdc_system/packages", {
		display   = display,
		letter    = letter,
		query     = query,
		install   = install,
...
function action_flashops()
	...
	elseif luci.http.formvalue("restore") then
		...
		if upload and #upload > 0 then
			luci.template.render("sdc_system/applyreboot")
			luci.sys.reboot()
		end
	elseif luci.http.formvalue("image") or luci.http.formvalue("step") then
		...
		if step == 1 then
			if image_supported() then
				luci.template.render("sdc_system/upgrade", {
				...
				fs.unlink(image_tmp)
				luci.template.render("sdc_system/flashops", {
...
function action_passwd()
	...
	luci.template.render("sdc_system/passwd", {stat=stat})
end

function action_reboot()
	local reboot = luci.http.formvalue("reboot")
	luci.template.render("sdc_system/reboot", {reboot=reboot})
	if reboot then
		luci.sys.reboot()
	end
end
